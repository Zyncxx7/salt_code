...
class Trader:
    def __init__(self):
        ...
        self.product_params = {'max_position' : dict(max_position), 
                               ...
                               'mm_spread':dict(mm_spread),
                               'mm_sensitivity':dict(mm_sensitivity), 
        ...
        self.product_strategy = {
                ...
                'CROISSANTS' : 'pair',        ##curentbest IDK
    ...
    def run(self, state: TradingState):
        ...
        for product in products:
            if product not in state.listings:
                continue
            if self.strategy[self.product_strategy[product]]=='pair':
                order = self.pair_trading_strategy('CROISSANTS', 'JAMS', state.position.get('CROISSANTS',0), state.position.get('JAMS',0),state.order_depths['CROISSANTS'], state.order_depths['JAMS'])
            else:
    ...
    def avellaneda(self, product, order_depth, current_position, timestamp):
        ...
        return orders
    def pair_trading_strategy(self, product_a, product_b, pos_a,pos_b, order_depth_a,order_depth_b):
        orders = []
        
        # Price histories
        history_a = list(self.product_params['price_history'][product_a])
        history_b = list(self.product_params['price_history'][product_b])

        # Mid-prices

        if not order_depth_a or not order_depth_b:
            return []

        mid_a = self.get_mid_price(product_a, order_depth_a)
        mid_b = self.get_mid_price(product_b, order_depth_b)

        if mid_a is None or mid_b is None:
            return []

        self.product_params['price_history'][product_a].append(mid_a)
        self.product_params['price_history'][product_b].append(mid_b)

        history_a = self.product_params['price_history'][product_a]
        history_b = self.product_params['price_history'][product_b]
        ...
        spread_series = [a - b for a, b in zip(list(history_a)[-20:], list(history_b)[-20:])]
        mean_spread = sum(spread_series) / len(spread_series)
        std_spread = statistics.stdev(spread_series)
        current_spread = mid_a - mid_b
        z_score = (current_spread - mean_spread) / std_spread if std_spread > 0 else 0

        # logger.print(f"[PAIR] {product_a}-{product_b} Spread: {current_spread:.2f}, Z: {z_score:.2f}")
        ...
        position_a = pos_a
        position_b = pos_b
        max_pos = self.product_params['max_position'][product_a]  # Assume both have same limit
        volume = int(max_pos)

        # Trade logic
        if abs(z_score) > threshold_open and position_a == 0 and position_b == 0:
            # Open a pair
            if z_score > 0:
                # A is overvalued → Short A, Long B
                orders.append(Order(product_a, int(mid_a), -volume))
                orders.append(Order(product_b, int(mid_b), volume))
            else:
                # B is overvalued → Long A, Short B
                orders.append(Order(product_a, int(mid_a), volume))
                orders.append(Order(product_b, int(mid_b), -volume))

        elif abs(z_score) < threshold_close and position_a != 0 and position_b != 0:
            # Close the pair
            orders.append(Order(product_a, int(mid_a), -position_a))
            orders.append(Order(product_b, int(mid_b), -position_b))
    ...
    def fair_price_mm_strategy(self, product, order_depth, current_position, timestamp):
        ...
        fair_price = self.get_mid_price(product,order_depth)
    ...
    def get_mid_price(self, product, order_depth,window_size=3):
        ...
        if strategy == 'true_value':
            ...
            bid_prices = sorted(candidate_bids, reverse=True)
            ...
            return self.product_params['ema'][product]
